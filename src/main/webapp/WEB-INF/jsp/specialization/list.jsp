...
<t:layout>
    <jsp:body>
        ...
        <a class="btn btn-primary" href="<c:url
               value="/Specjalizacja/nowy"/>">
            <span class="glyphicon glyphicon-plus"></span> Dodaj nową specjalność
        </a>
        <c:choose>
            <c:when test="${empty elements}">
                <div role="alert">			
                    <strong>Nie znaleziono żadnych specjalności.</strong>
                </div>
            </c:when>
            <c:otherwise>
                            ...
                            <tr>
                                <td>
                                    <a class="btn btn-link" href="<c:url
                                           value="/Specjalizacja/widok/${element.specializationId}"/>">
                                    ${element.name}</td>
                                <td><c:choose>
                                ...
                                <td>
                                    <!--Edytuj -->
                                    <a class="btn btn-link" href="<c:url
                                           value="/Specjalizacja/edycja/${element.specializationId}"/>">
                                ...
                                <td>
                                    ...
                                    <c:if test="${element.enabled}">
                                        <c:url var="zablokujUrl" value="/Specjalizacja/${element.specializationId}/zablokuj"/>
                                    ...
                                    <!--Odblokuj -->
                                    <c:if test="${!element.enabled}">
                                        <c:url var="odblokujUrl" value="/Specjalizacja/${element.specializationId}/odblokuj"/>
                                ...
                                <td>
                                    <!--Usuń -->
                                    <c:url var="usunUrl" value="/Specjalizacja/${element.specializationId}/usun"/>
                                    <form action="${usunUrl}" method="post"
                                          onsubmit="return confirm('Czy chcesz usunąć wybraną specjalność?') ? true : false;">                    
                                        <c:choose>
                ...
                <c:if test="${endIndex >1 }">                                            
                    <c:url var="firstUrl" value="/Specjalizacja/lista/1" />
                    <c:url var="lastUrl" value="/Specjalizacja/lista/${thePage.totalPages}" />
                    <c:url var="prevUrl" value="/Specjalizacja/lista/${currentIndex - 1}" />
                    <c:url var="nextUrl" value="/Specjalizacja/lista/${currentIndex + 1}" />
                        ...
                        <ul class="pagination">
                            <c:choose>
                                ...
                                <c:forEach var="i" begin="${beginIndex}" end="${endIndex}">
                                    <c:url var="pageUrl" value="/Specjalizacja/lista/${i}" />
                                    <c:choose>
